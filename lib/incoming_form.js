...
IncomingForm.prototype.handlePart = function(part)
{
	...
	var filePath = this.generateUploadPath(part.filename);
	...
	if(filePath)
	{
		...
		part.on("end", function()
		{
			...
			{
				self._flushing--;
				self.emit("fileEnd", part.name, file);
				self._maybeEnd();
			});
		});
	}
	else //if filePath is not a valid path, no file was creates and the uploaded bytes were lost.
	{
		part.on("end", function()
		{
			//self.emit("file", part.name, undefined);	//indicate that there was an error with the file and that it was not written
		});
	}
};
...
IncomingForm.prototype._initOctetStream = function()
{
	...
	else //invalid path, upload ignored
	{
		self._parser.on("end", function()
		{
			self._maybeEnd();
...
IncomingForm.prototype.generateUploadPath = function(filename)
{
	var name = "";
	for(var i = 0; i < 32; i++)
	{
